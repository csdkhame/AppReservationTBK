  ...
  public function customerRef_list($id_head,$code){
	...
	if($query->num_rows > 0) {
		 foreach($query->result() as $key=>$row){
			...
			$data[$key]['date_tran'] = $row->arrival_date." ".$row->arrival_time;
  ...
  public function customerRef_list_cus($num_record,$start){
	...
	if($query->num_rows > 0) {
		 foreach($query->result() as $key=>$row){
			...
			$tb_product = $db2->dbprefix('web_product');
			$db2_select = $db2->select('id,topic,cost_a,cost_b,cost_a_s,cost_b_s,category')->from($tb_product)->where('id',$row->product)->get()->row();
			
			$data_row[$key]['id_order'] = $row->id;
			$data_row[$key]['guest'] = $row->guest_english;
			$data_row[$key]['total_price'] = $row->total_price;
			$data_row[$key]['topic'] = $db2_select->topic;
			$data_row[$key]['category'] = $db2_select->category;
			$data_row[$key]['date_time'] = $row->arrival_date." ".$row->arrival_time;
